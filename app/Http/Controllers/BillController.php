...
public function approve($id)
{
    ...
    DB::transaction(function () use ($companyId, $bill, $lines, $period, $id) {
        ...
        // Debit VAT receivable if VAT exists
...
public function recordPayment(Request $request, $id)
{
    ...
    DB::transaction(function () use ($companyId, $bill, $amount, $request, $period, $id) {
        $count    = DB::table('journal_entries')->where('company_id', $companyId)->count() + 1;
        $jeNumber = 'JE-' . date('Y') . '-' . str_pad($count, 5, '0', STR_PAD_LEFT);
...
public function edit($id)
{
    ...
    $bill      = DB::table('bills')->where('id', $id)->where('company_id', $companyId)->first();
    if (!$bill) abort(404);
    if ($bill->status !== 'DRAFT') return redirect("/bills/{$id}")->with('error', 'Only draft bills can be edited.');
