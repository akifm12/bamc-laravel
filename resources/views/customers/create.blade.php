...
<form method="POST" action="/customers" class="space-y-4">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-5">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                <label class="text-xs text-gray-500 block mb-1">Customer Code *</label>
            ...
            <div>
                <label class="text-xs text-gray-500 block mb-1">TRN (Tax Registration No.)</label>
                <input type="text" name="trn" value="{{ old('trn') }}"
                    placeholder="100XXXXXXXXX00003"
                    class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
            </div>
            <div class="flex items-center gap-2 mt-2">
                <input type="checkbox" name="is_vat_registered" id="is_vat_registered"
                    {{ old('is_vat_registered') ? 'checked' : '' }} class="rounded">
            ...
            <div>
                ...
                <select name="ar_account_id" class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
                    ...
                    @foreach($accounts as $a)
                        <option value="{{ $a->id }}" {{ old('ar_account_id') == $a->id ? 'selected' : '' }}>
                            {{ $a->code }} - {{ $a->name }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="flex gap-3">
        ...
        <a href="/customers" class="text-sm text-gray-500 px-4 py-2 hover:text-gray-700">Cancel</a>
    </div>
