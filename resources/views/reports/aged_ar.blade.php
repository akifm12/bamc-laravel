...
<form method="GET" action="/reports/aged-ar" class="bg-white rounded-lg border border-gray-200 p-4 mb-4 flex gap-3 items-end">
    <div>
...
<!-- By Customer -->
@foreach($byCustomer as $customerName => $invList)
<div class="bg-white rounded-lg border border-gray-200 overflow-hidden mb-3">
    <div class="px-4 py-2 bg-gray-50 border-b border-gray-200 flex justify-between">
        <span class="font-semibold text-gray-700 text-sm">{{ $customerName }}</span>
        <span class="font-semibold text-sm text-red-600">AED {{ number_format($invList->sum('amount_due'), 2) }}</span>
    </div>
