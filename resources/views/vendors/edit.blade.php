...
<form method="POST" action="/vendors/{{ $vendor->id }}" class="space-y-4">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-5">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="name_arabic" value="{{ old('name_arabic', $vendor->name_arabic) }}"
            ...
            <div>
                ...
                <input type="text" name="trn" value="{{ old('trn', $vendor->trn) }}"
                    class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
            </div>
            <div class="flex items-center gap-4 mt-4">
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="is_vat_registered" id="is_vat_registered"
                        {{ $vendor->is_vat_registered ? 'checked' : '' }} class="rounded">
                    <label for="is_vat_registered" class="text-sm text-gray-600">VAT Registered</label>
                </div>
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="is_foreign_vendor" id="is_foreign_vendor"
                        {{ $vendor->is_foreign_vendor ? 'checked' : '' }} class="rounded">
            ...
            <div>
                ...
                <select name="emirate" class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
                    ...
                    @foreach($emirates as $e)
                        <option value="{{ $e }}" {{ $vendor->emirate == $e ? 'selected' : '' }}>{{ $e }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="text-xs text-gray-500 block mb-1">Country</label>
                <input type="text" name="country" value="{{ old('country', $vendor->country ?? 'UAE') }}"
                    class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-5">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="iban" value="{{ old('iban', $vendor->iban) }}"
                    class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
            </div>
            <div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="ap_account_id" class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
                    ...
                    @foreach($accounts as $a)
                        <option value="{{ $a->id }}" {{ $vendor->ap_account_id == $a->id ? 'selected' : '' }}>
                            {{ $a->code }} — {{ $a->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="payment_terms_days"
                    value="{{ old('payment_terms_days', $vendor->payment_terms_days ?? 30) }}"
                    class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
            </div>
        </div>
        <div class="mt-4">
            ...
            <textarea name="notes" rows="2"
                class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">{{ old('notes', $vendor->notes) }}</textarea>
        </div>
    </div>
    ...
    <div class="flex gap-3">
        ...
        <a href="/vendors/{{ $vendor->id }}" class="text-sm text-gray-500 px-4 py-2 hover:text-gray-700">Cancel</a>
    </div>
