...
<form method="POST" action="/bills/{{ $bill->id }}">
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-5 mb-4">
        <div class="flex items-center justify-between mb-4">
            <h2 class="font-semibold text-gray-700">Edit Bill - {{ $bill->bill_number }}</h2>
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="date" name="due_date" value="{{ $bill->due_date }}"
                    class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
            </div>
            <div>
                <label class="text-xs text-gray-500 block mb-1">Vendor Reference</label>
                <input type="text" name="vendor_ref" value="{{ $bill->vendor_ref }}"
                    class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
            </div>
            <div class="col-span-2">
                <label class="text-xs text-gray-500 block mb-1">Notes</label>
                <input type="text" name="notes" value="{{ $bill->notes }}"
                    class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 overflow-hidden mb-4">
        <div class="px-5 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-gray-400 mt-0.5">Select the expense account for each line. AP account is assigned automatically.</p>
            </div>
    ...
    <div class="flex gap-3">
        ...
        <a href="/bills/{{ $bill->id }}" class="text-sm text-gray-500 px-4 py-2 hover:text-gray-700">Cancel</a>
    </div>
