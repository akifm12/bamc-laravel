    ...
    <div class="bg-white rounded-lg border border-gray-200 p-5 mb-4">
        ...
        <div class="mt-3 grid grid-cols-3 gap-3 text-sm">
            <div>
                ...
                <p class="font-semibold">AED {{ number_format($bill->total_amount, 2) }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-400">Paid So Far</p>
                <p class="font-semibold text-green-600">AED {{ number_format($bill->amount_paid, 2) }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-400">Amount Due</p>
                <p class="font-semibold text-red-600">AED {{ number_format($bill->amount_due, 2) }}</p>
            </div>
        </div>
    </div>

    <form method="POST" action="/bills/{{ $bill->id }}/payment"
        ...
        <div>
            ...
            <input type="number" name="amount" value="{{ number_format($bill->amount_due, 2, '.', '') }}"
                step="0.01" min="0.01" required
                class="w-full border border-gray-200 rounded px-3 py-1.5 text-sm">
        </div>

        <div>
            <label class="text-xs text-gray-500 block mb-2">Paid From *</label>
